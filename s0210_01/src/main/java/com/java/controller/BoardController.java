...
//		bdto.getBno(),bdto.getBtitle(),bdto.getBcontent(),bdto.getId()));	
		model.addAttribute("board", bdto); //객체 통째로 전달
...
//		model.addAttribute("id", bdto.getId());
	...
	// 3. 축약방식
	// @RequestParam 방식 - 이름이 같으면 이름 생략
	// 자동으로 형변환 가능 - 타입이 다른경우는 null값일때 에러발생
	//@RequestParam(name="bno",defaultValue ="1")
//	@PostMapping("/board")
//	public String board(
//			//앞에 @RequestParam 생략 가능 : 단, 뒤에 이름 두개가 같아야함
//			//@RequestParam("bno") int bno, : 원래 형태 
//			int bno,           //("bno")생략가능 :뒤에 bno와 이름이 같아서 // null값:에러(int라서)
//			String btitle,     //("btitle")생략가능
//			String bcontent,   //("bcontent")생략가능
//			String id,         //("id")생략가능
//			Model model) {
...
//			Model model) {
//		System.out.println(String.format("%s,%s,%s,%s", bno,btitle,bcontent,id));
//		model.addAttribute("bno", bno);
//		model.addAttribute("btitle", btitle);
//		model.addAttribute("bcontent", bcontent);
//		model.addAttribute("id", id);
